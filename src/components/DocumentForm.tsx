...
export const DocumentForm: React.FC<DocumentFormProps> = ({ initialData, onSave }) => {
  ...
  useEffect(() => {
    ...
    if (initialData) {
      ...
      setDocumentNumber(`${documentType === 'invoice' ? 'R' : 'A'}-${year}${month}${day}-${time}`);
    }
    ...
    <div className="max-w-6xl mx-auto p-6">
      <div className="mb-8">
        ...
        <p className="text-gray-600">
          Erstellen Sie professionelle {documentType === 'invoice' ? 'Rechnungen' : 'Angebote'} mit Drag-and-Drop-Funktionalität
        </p>
      </div>
      ...
      {/* Notes */}
      <div className="bg-white rounded-lg shadow-sm border p-6 mb-8">
        <h2 className="text-xl font-semibold mb-4">Anmerkungen</h2>
        <textarea
          value={notes}
          ...
          placeholder="Zusätzliche Informationen oder Zahlungsbedingungen..."
        />
      </div>
      ...
      <div className="flex flex-wrap gap-4 justify-end">
        ...
        <PDFDownloadLink
          ...
          fileName={`${documentType === 'invoice' ? 'Rechnung' : 'Angebot'}-${documentNumber}.pdf`}
